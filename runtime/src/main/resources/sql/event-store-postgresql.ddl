...
BEGIN
  ...
  CREATE INDEX IF NOT EXISTS t_event_aggregateRootIdentifier ON T_EVENT USING BTREE (aggregaterootid, aggregateroottype);
  ...
  ELSE
    ...
    BEGIN
      ...
      IF NEW.version = 0 AND event_count > 0 THEN
        RAISE EXCEPTION 'Event already present while should not be ! aggregaterootid %s aggregateroottype %s', NEW.aggregaterootid, NEW.aggregateroottype;
      END IF;
      ...
      IF last_version <> NEW.version - 1 THEN
        RAISE EXCEPTION 'Previous event version mismatch - current version % - previous version %', NEW.version, last_version;
      END IF;
      expected_version = last_version + 1;
      IF NEW.version != expected_version THEN
        RAISE EXCEPTION 'version unexpected % - current version % - expected version %', NEW.owner, NEW.version, expected_version;
      END IF;
      RETURN NEW;
    END;
